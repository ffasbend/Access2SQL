...
def export_queries(accdb: Path, fmt: str = "txt") -> Path:
    ...
    if fmt == "md":
        lines: list[str] = [
            f"# Queries — {accdb.stem}",
            "",
            f"Generated by access2sql.py v{VERSION}  ",
            f"Source: `{accdb.parent.name}/{accdb.name}`",
            "",
        ]
        if not query_names:
            ...
            for query_name in query_names:
                ...
                try:
                    ...
                    lines.append("```sql")
                    lines.append(sql if sql else "-- (empty)")
                    lines.append("```")
...
def main():
    print(f"Access → SQLite extractor  v{VERSION}")
    print("=" * 50)
